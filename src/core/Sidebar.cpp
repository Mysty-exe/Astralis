...
Sidebar::Sidebar(SDL_Renderer *renderer, int width, int height)
{
    ...
    velocityHelperTxt.loadFromRenderedText(renderer, smallFont, "Ex: [100000] or [1e+5] km/s", {211, 211, 211});
    ...
    msTxt.loadFromRenderedText(renderer, smallFont, "km/s", {255, 255, 255});
    ...
    addRect = {0 - (int)addTransitionOffset, 0, width / 3, height};
    editRect = {(int)(width - (width / 3)) - (int)editTransitionOffset, 0, width / 3, height};
    headerRect = {0 - (int)addTransitionOffset, 0, sidebarWidth, (int)sidebarTxt.getHeight() + 20};
    color = {255, 255, 255};
    setFrames();
}
...
void Sidebar::viewObjects(Events events, Simulation &sim, Vector &panningOffset, double deltaTime)
{
    ...
    {
        SDL_Rect rect = {(int)(20 - addTransitionOffset), (int)(startY - viewScroll), (int)(sidebarWidth - 40), 65};
...
void Sidebar::addObject(Events events, Vector spawnLoc, Simulation &sim, double deltaTime)
{
    SDL_Point point = events.getPoint();
    ...
    for (Frame &frame : frames)
    {
        ...
        if (SDL_PointInRect(&point, &rect) && !SDL_PointInRect(&point, &headerRect))
        {
            if (events.leftClick && SDL_PointInRect(&events.startClickPos, &rect))
            {
                ...
                editingObject = sim.objects.size() - 1;
                setInputs(sim, sim.objects[sim.objects.size() - 1]);
                addActivated = false;
                editActivated = true;
                editState = true;
...
void Sidebar::editObject(Events events, Vector spawnLoc, Simulation &sim, double deltaTime)
{
    ...
    else if (events.leftClick & SDL_PointInRect(&point, duplicateBtn.getRect()) && SDL_PointInRect(&events.startClickPos, duplicateBtn.getRect()))
    {
        if (!nameInput.error && !massInput.error && !radiusInput.error && !velocityInput.error && !degreeInput.error)
        {
            if (editingObject != -1)
            {
                ...
                CelestialObject obj = CelestialObject(renderer, width, height, name, sim.objects[editingObject].objType, sim.objects[editingObject].folder, sim.objects[editingObject].mass, sim.objects[editingObject].radius, spawnLoc.x, spawnLoc.y, sim.objects[editingObject].velocity);
                obj.ctrl = true;
                obj.editing = true;
                sim.objects.push_back(obj);
                editingObject = sim.objects.size() - 1;
...
void Sidebar::runSidebar(Events events, Vector spawnLoc, Simulation &sim, Vector &panningOffset, double timeStep)
{
    SDL_Point point = events.getPoint();
    ...
    if (editingObject != -1)
    {
        if (events.checkSpecialHoldKey(W))
        {
            sim.objects[editingObject].position.y -= 0.01 * timeStep;
        }
        else if (events.checkSpecialHoldKey(S))
        {
            sim.objects[editingObject].position.y += 0.01 * timeStep;
        }
        if (events.checkSpecialHoldKey(A))
        {
            sim.objects[editingObject].position.x -= 0.01 * timeStep;
        }
        else if (events.checkSpecialHoldKey(D))
        {
            sim.objects[editingObject].position.x += 0.01 * timeStep;
        }
    }
    ...
    {
        if (sim.objects.size() > 0 && selectedObject != -1)
        {
    ...
    if (addState || viewState)
    {
        ...
        if (viewState)
        {
            viewObjects(events, sim, panningOffset, timeStep);
        }
        else
        {
            addObject(events, spawnLoc, sim, timeStep);
        }
    ...
    if (editState)
    {
        if (events.holdingClick && SDL_PointInRect(&point, circle.getRect()))
        {
        ...
        {
            editObject(events, spawnLoc, sim, timeStep);
            updateObject(events, sim, timeStep);
        }
        SDL_SetRenderDrawColor(renderer, 177, 156, 217, 255);
...
void Sidebar::updateObject(Events events, Simulation &sim, double deltaTime)
{
    if (events.input.length() > 0 || events.checkSpecialKey(BACKSPACE) || events.checkSpecialKey(SPACE) || sliderActivated)
    {
        ...
        if (Utilities::validateVelocity(velocityPair.first) && Utilities::validateVelocity(velocityPair.second))
        {
            ...
            if (v != sim.objects[editingObject].velocity.magnitude() / Utilities::getTimeRates()[sim.timeRate].second)
            {
                if (sim.objects[editingObject].velocity.magnitude() != 0)
                {
                    sim.objects[editingObject].velocity = sim.objects[editingObject].velocity.normalize() * sim.scaleDistance(v * Utilities::getTimeRates()[sim.timeRate].second);
                }
                else
                {
                    sim.objects[editingObject].velocity = Vector(0, sim.scaleDistance(v * Utilities::getTimeRates()[sim.timeRate].second));
                }
            }
        }
        else
...
void Sidebar::setInputs(Simulation sim, CelestialObject &object)
{
    ...
    ssVelocity << sim.getRealDistance(object.velocity.magnitude() / Utilities::getTimeRates()[sim.timeRate].second);
...
void Sidebar::reset()
{
    ...
    editTransitionOffset = -(width / 3);
    addRect = {0 - (int)addTransitionOffset, 0, width / 3, height};
    editRect = {(int)(width - (width / 3)) - (int)editTransitionOffset, 0, (int)(width / 3), (int)height};
    headerRect = {0 - (int)addTransitionOffset, 0, sidebarWidth, (int)sidebarTxt.getHeight() + 20};
}
