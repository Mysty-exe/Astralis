...
SimulationHandler::SimulationHandler(SDL_Renderer *renderer, int width, int height)
{
    ...
    binIcon.loadFromFile(renderer, "assets/binIcon.png", 0.5);
    binHoverIcon.loadFromFile(renderer, "assets/binHoverIcon.png", 0.5);
    ...
    renderQuad = {0 - (int)panningOffset.x, 0 - (int)panningOffset.y, width, height};
...
void SimulationHandler::saveSimulationData()
{
    ...
    for (Simulation sim : simulations)
    {
        ...
        {
            obj.velocity /= Utilities::getTimeRates()[sim.timeRate].second;
...
string SimulationHandler::newSimulation(Events events, string state, double timeStep)
{
    ...
    if (transition.transitionState == "Out" || transition.transitionState == "Done")
    {
        ...
        SDL_Rect newRect = {100, (int)(newSimulationTxt.y + newSimulationTxt.getHeight() + 20), (int)width - 200, 70};
        ...
        {
            SDL_SetRenderDrawColor(renderer, 203, 195, 227, 255);
            SDL_Rect templateRect = {100, (int)(startY - newSimScroll), (int)width - 200, 70};
        ...
        SDL_SetRenderDrawColor(renderer, 0, 0, 0, 255);
        SDL_Rect blackRect = {0, 0, (int)width, (int)(templateTxt.y + templateTxt.getHeight() + 10)};
...
string SimulationHandler::loadSimulation(Events events, string state, double timeStep)
{
    ...
    if (transition.transitionState == "Out" || transition.transitionState == "Done")
    {
        ...
        for (int i = 0; i < simulations.size(); i++)
        {
            ...
            binIcon.setCoords(width - 100 - binIcon.getWidth() - 10, startY + 70 - loadSimScroll - binIcon.getHeight() - 10);
            binHoverIcon.setCoords(width - 100 - binIcon.getWidth() - 10, startY + 70 - loadSimScroll - binIcon.getHeight() - 10);
            editIcon.setCoords(binIcon.x - editIcon.getWidth() - 10, startY + 70 - loadSimScroll - editIcon.getHeight() - 10);
            editHoverIcon.setCoords(binIcon.x - editIcon.getWidth() - 10, startY + 70 - loadSimScroll - editIcon.getHeight() - 10);
            simNameTxt.render(renderer);

            SDL_Rect rect = {100, (int)(startY - loadSimScroll), (int)width - 200, 70};
            ...
            if (SDL_PointInRect(&point, &rect))
            {
                ...
                if (events.leftClick && SDL_PointInRect(&events.startClickPos, &rect) && SDL_PointInRect(&point, &rect) && !SDL_PointInRect(&point, binIcon.getRect()))
                {
                    Mix_PlayChannel(-1, click2Sound, 0);
                    currentSimulation = i;
                    simulations[currentSimulation].runTrajectoryThread();
                    state = "Simulation";
                    break;
                }
            }
        ...
        SDL_SetRenderDrawColor(renderer, 0, 0, 0, 255);
        SDL_Rect rect = {0, 0, (int)width, (int)(loadSimulationTxt.y + loadSimulationTxt.getHeight() + 5)};
...
string SimulationHandler::editSimulation(Events events, string state, double timeStep)
{
    ...
    if (transition.transitionState == "Out" || transition.transitionState == "Done")
    {
        ...
        if (SDL_PointInRect(&point, nameInput.getRect()) || SDL_PointInRect(&point, distRatioInput.getRect()) || SDL_PointInRect(&point, simRadiusInput.getRect()) || SDL_PointInRect(&point, confirmBtn.getRect()) || events.checkSpecialKey(ENTER))
        {
            if ((events.leftClick && SDL_PointInRect(&events.startClickPos, confirmBtn.getRect()) || events.checkSpecialKey(ENTER)))
            {
                state = handleEdits(events, distRatioPair, simRadiusPair, state);
            }
            if (!events.checkSpecialKey(ENTER))
            {
                pointer = true;
            }
        }
...
void SimulationHandler::resetSimulation()
{
    ...
    {
        obj.velocity /= Utilities::getTimeRates()[simulations[currentSimulation].timeRate].second;
    }
    pause = false;
    editing = false;
    view = false;
    ...
    distance = false;
    panningOffset = Vector(width / 2, height / 2);
    simulations[currentSimulation].timeRate = 0;
...
{
    SDL_Rect rect = {(int)(width - (width / 4)), 0, (int)(width / 4), (int)(height * 2 / 3)};
    simulations[currentSimulation].displayObjectInfo(simulations[currentSimulation].objects[focusedObject], width - (width / 4), 0);
}
...
void SimulationHandler::viewState(Events events, double timeStep)
{
    ...
    SDL_Rect rect = {(int)(0 - viewStart), (int)startY, (int)(width / 4), (int)(width / 4)};
...
void SimulationHandler::mainScreen(Events events, double timeStep)
{
    ...
    if (!pause && !editing && simulations[currentSimulation].state != RETURN)
    {
        ...
        simulations[currentSimulation].applyForces(timeStep);
    ...
    SDL_SetRenderDrawColor(renderer, 177, 156, 217, 255);
    simulations[currentSimulation].display(&renderQuad, panningOffset, timeStep);
    ...
    if (!view)
    {
        simulations[currentSimulation].displaySimulationDate(height);
        simulations[currentSimulation].displayTimeRate();
    }
    ...
    renderQuad = {0 - (int)panningOffset.x, 0 - (int)panningOffset.y, (int)width, (int)height};
}
...
void SimulationHandler::editScreen(Events events, double timeStep)
{
    ...
    sidebar.runSidebar(events, spawnLoc, simulations[currentSimulation], panningOffset, timeStep);
    ...
    {
        SDL_Rect obj1Rect = {(int)(obj1.position.x + panningOffset.x - obj1.radius), (int)(obj1.position.y + panningOffset.y - obj1.radius), (int)(obj1.radius * 2), (int)(obj1.radius * 2)};
        ...
        {
            SDL_Rect obj2Rect = {(int)(obj2.position.x + panningOffset.x - obj2.radius), (int)(obj2.position.y + panningOffset.y - obj2.radius), (int)(obj2.radius * 2), (int)(obj2.radius * 2)};
...
void SimulationHandler::eventsHandler(Events events)
{
    ...
    {
        if (simulations[currentSimulation].timeRate < Utilities::getTimeRates().size() - 1)
        {
            simulations[currentSimulation].timeRate++;
            for (CelestialObject &co : simulations[currentSimulation].objects)
            {
                co.velocity *= Utilities::getTimeMultipliers()[simulations[currentSimulation].timeRate].second;
            }
        }
    }
    else if (events.checkSpecialKey(LEFT) && !pause && simulations[currentSimulation].state != RETURN && !view)
    {
        if (simulations[currentSimulation].timeRate > 0)
        {
            for (CelestialObject &co : simulations[currentSimulation].objects)
            {
                co.velocity /= Utilities::getTimeMultipliers()[simulations[currentSimulation].timeRate].second;
